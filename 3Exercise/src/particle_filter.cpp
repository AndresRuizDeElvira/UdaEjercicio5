...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
    ...
    //mean(x,y, theta), stddev(std[0],std[1],std[2]
    //std::vector<Particle> particles;	// This line creates a
    ...
    //now, we initialize all particles following each point in the gaussian distribution
    //we need a for loop for each position
    
	for (int i = 0; i < num_particles; ++i) {
		double sample_x = dist_x(gen);
		double sample_y = dist_y(gen);
		double sample_theta = dist_theta(gen);
        //Random selection of points using generator, which returns a value, which is select for the distribution
        //we create then a particle
        //std::vector<Particle> particles;
        //struct Particle {    int id; double x;     double y; double theta; double weight;    std::vector<int> associations;
		...
		particle.weight = 1.0;
        //we then push back the particle we have just created into the vector
        particles.push_back(particle);
	}
    //Now all particles have been initialized therefore is_initialized
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
	...
	for (auto& p : particles) {
		if (fabs(yaw_rate) > 0.00001) {
            //yaw_rate is not 0, therefore we must include all angles as follows
            //we have to use the formulas for angle not changed
            double velocity_by_yaw_rate = velocity/yaw_rate;
            //the speed of change is the velocity divided by yaw_rate
            double next_theta = p.theta + yaw_rate * delta_t;
            //next angle is current angle theta +rate of change yaw by the time change
			p.x += velocity_by_yaw_rate * (sin(next_theta) - sin(p.theta));
            //value of x value of particle is current+velocity/yaw_rate*(sin(p.theta+yaw_rate*delta_t)-sin(p.theta))
			p.y += velocity_by_yaw_rate * (cos(p.theta)  - cos(next_theta));
            //the same is done for y values
			p.theta = next_theta;
            //finally the particle prediction angles is set to the next angle
            
		} else {
            //yaw_rate is 0, or close no- yaw rate
		...
		// add noise
        //adding noise as seen in the lessons using std
		normal_distribution<double> dist_x(p.x, std_pos[0]);
        // normal (Gaussian) distribution for x
		normal_distribution<double> dist_y(p.y, std_pos[1]);
        // normal (Gaussian) distribution for y
...
vector<LandmarkObs> ParticleFilter::dataAssociation(vector<LandmarkObs> predicted, vector<LandmarkObs>& observations) {
    ...
    //we create landmark vector as follows
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
	...
	for (int i=0; i < particles.size(); ++i) {
		...
		for (auto observation: observations){
            ...
            //transformation as follows: p.x+(observation.x*cos(p.theta))-(observation.y*sin(p.theta)); on X.
